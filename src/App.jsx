...
export default function App() {
  ...
  return <>
    ...
    <header className="topbar"><div className="topbar-inner">
      <a href="#explore" className="brand" aria-label="Closet Prayer Study home"><span className="brand-icon"><BookOpen size={26} strokeWidth={1.7} /></span><span><strong>Study</strong><small>Closet Prayer</small></span></a>
      <div className="header-actions"><span className="development">Development</span><span className={`save-indicator ${saveState === 'error' ? 'error-text' : ''}`} title="Notebook storage status">{locked ? <LockKeyhole size={15} /> : saveState === 'saved' ? <ShieldCheck size={15} /> : <Clock3 size={15} />}<span>{locked ? 'Locked' : saveState === 'saved' ? 'Saved on this device' : saveState === 'error' ? 'Save failed' : 'Saving…'}</span></span>{protectedData && !locked && <button className="icon-button" onClick={lock} aria-label="Lock notebook" disabled={busy}><LockKeyhole size={19} /></button>}<button className="icon-button" onClick={switchTheme} aria-label={`Use ${theme === 'dark' ? 'light' : 'dark'} mode`}>{theme === 'dark' ? <Sun size={21} /> : <Moon size={21} />}</button></div>
    </div></header>
    ...
    {!online && <div className="offline-bar">You’re offline. Your saved study material and notebook are available on this device.</div>}
    ...
    {!locked && <nav className="bottom-nav" aria-label="Main navigation"><div>{[['explore', 'Explore', Search], ['journal', 'My journal', NotebookPen], ['settings', 'Settings', SettingsIcon]].map(([id, label, Component]) => <a key={id} href={`#${id}`} aria-current={active === id ? 'page' : undefined}><Component size={22} strokeWidth={1.8} /><span>{label}</span></a>)}</div></nav>}
    ...
    {pendingDraft && <Modal title="You have an unfinished reflection" onClose={() => setPendingDraft(null)}><div className="modal-body"><p>Your existing draft is saved on this device. Finish or discard it before beginning another.</p><div className="actions"><button className="primary" onClick={() => { setPendingDraft(null); setEditor(true); }}>Continue my draft</button><button onClick={() => setPendingDraft(null)}>Keep exploring</button></div></div></Modal>}
...
function Explore({ query, setQuery, topic, setTopic, notebook, openStudy, startDraft, resume, navigate }) {
  ...
  return <div className="explore-page">
    ...
    <div className="explore-layout"><div>
      ...
      <p className="local-note"><LockKeyhole size={14} />Your notes stay on this device.</p>
    </aside></div>
  </div>;
}
...
function Settings({ notebook, protectedData, busy, run, flush, change, current, setProtected, setRecoveryCode, setConfirm, setNotice, exportBackup, exportText, lock, theme, switchTheme }) {
  ...
  return <div className="settings-page"><span className="eyebrow">YOUR STUDY SPACE</span><h1>Settings</h1>
    ...
    <section className="settings-section"><h2><ShieldCheck size={21} />Private Vault</h2><p>Your research stays in this browser. Private Vault also encrypts its contents with a passphrase and Recovery Code.</p>
